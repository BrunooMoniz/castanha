...
class TestEscolhaDoTranscritor(unittest.TestCase):
    ...
    def test_sem_groq_e_sem_vps_e_falha_declarada_nao_mock(self):
        ...
        with patch("castanha.transcription.subprocess.run", return_value=subprocess.CompletedProcess([], 255)):
            with self.assertRaises(RuntimeError) as ctx:
                get_transcriber(60)
        self.assertIn("Nenhum transcritor", str(ctx.exception))
...
class TestVpsTimeout(unittest.TestCase):
    def test_scp_travado_tem_teto_e_limpa(self):
        ...
        self.assertIn("Envio", str(ctx.exception))
        limpa.assert_called_once()

    def test_whisper_com_erro_limpa_o_tmp_remoto(self):
        from castanha.transcription import VpsSshTranscriber

        def fake_run(cmd, **kw):
            if cmd[0] == "scp":
                return subprocess.CompletedProcess(cmd, 0, "", "")
            return subprocess.CompletedProcess(cmd, 1, "", "whisper morreu")
